#!/usr/bin/env npx tsx
/**
 * Generate a photorealistic interior lifestyle hero via OpenRouter Nano Banana.
 *
 * Usage:
 *   npm run generate-interior-image -- african-violet "African Violet"
 *   npm run generate-interior-image -- --all
 *   npm run generate-interior-image -- spider-plant --reference ./my-photo.jpg
 */

import fs from "fs";
import path from "path";
import matter from "gray-matter";
import { generateInteriorPhoto } from "./lib/image-gen";
...
async function main() {
  ...
  if (all) {
    ...
    if (!slug) {
      console.error(
        "Usage: npm run generate-interior-image -- <slug> [Title]\n" +
          "       npm run generate-interior-image -- --all\n" +
          "       npm run generate-interior-image -- african-violet --reference ./photo.jpg",
      );
      process.exit(1);
    }
  ...
  for (const job of jobs) {
    console.log(`Generating interior photo for ${job.slug} (${job.title})…`);
    try {
      const { outputPath, usedReferencePhoto } = await generateInteriorPhoto({
        slug: job.slug,
        plantName: job.title,
      ...
      console.log(
        `  ✓ Saved ${outputPath}${usedReferencePhoto ? "" : " (prompt only)"}`,
      );
      ok++;
    } catch (err) {
